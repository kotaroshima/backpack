    ...
    if error
      ...
      exec 'coffee -j Backpack.js -c ./coffee/Backpack.coffee ./coffee/plugins/AttachPlugin.coffee ./coffee/plugins/SubscribePlugin.coffee ./coffee/plugins/PublishPlugin.coffee', (error, stdout, stderr)->
        if error
          ...
          exec 'coffee -j Backpack-all.js -c ./coffee/Backpack.coffee ./coffee/plugins/*.coffee ./coffee/components/ActionView.coffee ./coffee/components/StackView.coffee ./coffee/components/ListView.coffee ./coffee/components/EditableListView.coffee ./coffee/components/TabView.coffee ./coffee/components/GoogleMapView.coffee', (error, stdout, stderr)->
            if error
              util.log('Build fail : '+error)
            else
...
task 'build-css', 'Builds SCSS files into CSS files', (options)->
  util.log('Start build...')
  exec 'scss scss/ActionView.scss css/ActionView.css -t expanded; scss scss/EditableListView.scss css/EditableListView.css -t expanded; scss scss/TabView.scss css/TabView.css -t expanded', (error, stdout, stderr)->
    if error
      util.log('Build fail : '+error)
    else
      util.log('Build success')
